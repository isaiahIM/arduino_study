...
//    firstLoop=1;
    ...
    /*SPI read mode*/
    if(spi_readTask==SPI_READ_R)
    {
      sequenceNextState=SPI_WriteByte(CLK, MOSI, SPI_READ, timeCount, &bitCount);
    ...
    /*word address*/
    else if(spi_readTask==SPI_WORD_ADDRESS_R)
    {
      sequenceNextState=SPI_WriteByte(CLK, MOSI, wordAddr, timeCount, &bitCount);
    ...
    else if(spi_readTask==SPI_READ_DATA_R)
    {
      sequenceNextState=SPI_ReadByte(CLK,MOSI, MISO, timeCount, &bitCount, &readVal);
    }
    //readVal = SPI.transfer(0x00);
